...
def generate_payloads(payloads_file="payloads.txt"):
    """Generate LFI payloads from external file ONLY (no internal generation)"""
    payloads = []
    
    # Load external payloads from file only
    external_payloads = _load_external_payloads(payloads_file)
    payloads.extend(external_payloads)
    
    if not payloads:
        cprint("[!] No LFI payloads found in payloads.txt, add [LFI] section", "yellow")
